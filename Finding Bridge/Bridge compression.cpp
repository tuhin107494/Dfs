...
///====================================StartHere=================////////////
ll cs=0;
bitset<mx5>visited,bit;
vector<pll>v[mx5];
ll depth[mx5],bridge_id[mx5], low[mx5],col[mx5];
vector<pll>edges,isbridge;
void dfs(ll u,ll d,ll p)
{
    ...
    for(pll node:v[u])
    {
        ll to=node.F,id=node.S;

        if(node.F==p)continue;
        if(visited[node.F]==0)
        {
            dfs(to,d+1,u);
        ...
        {
            if(depth[to]<depth[u])low[u]=min(depth[to],low[u]);
        }
    }
